...
export function MicSelector({
  ...
  return (
    <DropdownMenu onOpenChange={handleDropdownOpenChange}>
      <DropdownMenuTrigger render={<Button variant="ghost" size="sm" className={cn(
                      "hover:bg-accent flex w-48 cursor-pointer items-center gap-1.5",
                      className
                    )} disabled={loading || disabled} />}>{isMuted ? (
                      <MicOff className="h-4 w-4 flex-shrink-0" />
                    ) : (
                      <Mic className="h-4 w-4 flex-shrink-0" />
                    )}<span className="flex-1 truncate text-left">
                      {currentDevice.label}
                    </span><ChevronsUpDown className="h-3 w-3 flex-shrink-0" /></DropdownMenuTrigger>
